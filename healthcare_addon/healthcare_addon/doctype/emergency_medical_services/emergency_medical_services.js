...
frappe.ui.form.on("Emergency Medical Services", {
  refresh(frm) {
    ...
    frm.fields_dict["imaging_tests"].grid.get_field(
...
frappe.ui.form.on("Emergency Medical Services", {
  emergency_services_template: function (frm) {
    if (frm.doc.emergency_services_template) {
      frappe.db.get_doc("Emergency Services Template", frm.doc.emergency_services_template).then((doc) => {
		frm.set_value("service_item", doc.services_item);
  ...
  setup: function (frm) {
    frappe.db
      .get_single_value(
        ...
        "emergency_medical_services_item"
      )
      .then((item) => {
      ...
      if (frm.doc.docstatus === 1) {
        if (
          ...
          if (frm.doc.inpatient_status != "Discharged") {
            frm
              .add_custom_button(__("Discharge Now"), function () {
                discharge_now(frm);
              })
              .addClass("btn-danger");
          }
...
let discharge_now = function (frm) {
  frappe.db
    ...
    .then((r) => {
      frappe.db
        .set_value(frm.doctype, frm.doc.name, "inpatient_status", "Discharged")
        .then(frm.refresh());
    });
