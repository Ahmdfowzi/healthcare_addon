frappe.ui.form.on("X-ray", {
  refresh(frm) {
    if (frm.is_new()) {
      frm.add_custom_button(__("Get (X-ray) Test"), function () {
        show_imaging_tests_dialog(frm, "X-ray");
      });
    }
  },
});
    ...
    callback: function (r) {
      if (r.message) {
        ...
        let imaging_tests_html = r.message
          .map(
            (test) => `
                  <div class="card imaging-test-card" data-imaging-scan-template="${
                    test.imaging_scan_template
                  }" data-document-source="${test.parent_docname}" data-invoiced="${
              test.invoiced
                      ...
                      <div class="card-body">
                          ...
                          <p class="card-text"><strong>${
                            test.invoiced ? __("Invoiced") : __("Not Invoiced")
                          }</strong></p>
                      </div>
                  </div>
              `
