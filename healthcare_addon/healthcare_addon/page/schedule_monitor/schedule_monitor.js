...
// 	return time12;
//   }
// frappe.pages['schedule-monitor'].on_page_load = async function (wrapper) {
...
// 	},1000)
// 	if(data.length > 0){
// 		parent.html(frappe.render_template("schedule_monitor",{appointments : data}));
// 	}else{
// 		parent.html(frappe.render_template("schedule_monitor",{appointments : null}));
// 	}
// }


frappe.pages['schedule-monitor'].on_page_load = async function (wrapper) {
	let parent = $(`<div class="app mx-5"></div>`).appendTo(wrapper);

	const data = await frappe.db.get_list("Patient Appointment", {
		fields: '*',
		filters: [["appointment_date", "=", frappe.datetime.now_date()], ['status', '!=', 'Closed']],
	});
	...
	const renderAppointments = (appointments) => {
		if (appointments.length > 0) {
			parent.html(frappe.render_template("schedule_monitor", { appointments }));
			setAppointmentTimers(appointments);
		} else {
			parent.html('<h1>No appointments yet!</h1>');
		}
	};
	...
	const setAppointmentTimers = (appointments) => {
		appointments.forEach(appointment => {
			const time = formatTime(appointment.appointment_time);
			const elm = document.querySelector(`[data-appointment="${appointment.name}"]`);

			if (isTimeLessThanNow(time)) {
				elm.classList.add("active");
			}
...
function isTimeLessThanNow(timeStr) {
	...
	const givenTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), hours, minutes, seconds);
